...
extension NfcManagerPlugin: NFCTagReaderSessionDelegate {
  ...
  public func tagReaderSession(_ session: NFCTagReaderSession, didDetect tags: [NFCTag]) {
    session.connect(to: tags.first!) { error in
      if let error = error {
        // Connect can fail for ISO-DEP / weak coupling. Keep polling so UID-only
        // clients can retry without invalidating the session.
        print(error)
...
private func convert(_ value: NFCNDEFTag, _ completionHandler: @escaping (TagPigeon?, Error?) -> Void) {
  ...
  // UID-only: identifier is already on the pigeon. Upstream nfc_manager 4.1.1
  // calls queryNDEFStatus/readNDEF here and drops the entire tag on failure
  // (empty NDEF, ISO-DEP/Type 4 without NDEF). That hides a valid serial.
